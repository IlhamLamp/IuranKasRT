                ...
                <tbody>
                    ...
                    <?php if ($warga) : foreach ($warga as $row) : ?>
                            <tr>
                                ...
                                <td><?= $row['no_rumah']; ?></td>
                                <td>
                                    <a class="btn btn-warning p-1 btn-sm" href="/Warga/detail/<?= $row['slug']; ?>">Aktif</a>
                                </td>
                                <td>
                                    <a class="btn btn-primary btn-sm m-1" href="<?= ('/Warga/edit/' . $row['id']); ?>"><i class="fas fa-edit"></i></a>
                                    <a class="btn btn-danger btn-sm m-1" href="<?= ('/Warga/delete/' . $row['id']); ?>" onclick="return confirm('Yakin menghapus data <?= $row['nama']; ?>?');"><i class="fa-solid fa-trash"></i></a>
                                </td>
                            </tr>
                        <?php endforeach;
                    else : ?>
